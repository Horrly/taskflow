...
function ActivityItem({ entry, showTaskTitle, onClick, isLast }) {
  ...
  return (
    ...
    >
      {!isLast && (
        <span className="absolute left-[13px] top-8 bottom-0 w-px bg-gray-100" />
      )}
      ...
      <div className="flex-1 min-w-0 pt-0.5">
        <p className={`text-sm text-gray-700 ${onClick ? 'group-hover:text-indigo-700 transition-colors' : ''}`}>
          <span className="font-semibold text-gray-900">{actorName}</span>{' '}
          {entry.verb}
          {entry.detail && <span> {entry.detail}</span>}
          {showTaskTitle && entry.task_title && (
            <span className="text-gray-400"> · {entry.task_title}</span>
          )}
        </p>
        <p className="text-xs text-gray-400 mt-0.5">{entry.human_time}</p>
      </div>
    </div>
  );
}
...
export default function ActivityFeed({
  ...
  if (loading) {
    return <p className="text-xs text-gray-400 py-4">Loading…</p>;
  }

  if (entries.length === 0) {
    return <p className="text-xs text-gray-400 py-4">{emptyMessage}</p>;
  }
    ...
    <div>
      ...
      {paginated && nextUrl && (
        <button
          ...
          className="mt-2 w-full py-2 text-xs font-medium text-indigo-600 hover:text-indigo-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50"
        >
